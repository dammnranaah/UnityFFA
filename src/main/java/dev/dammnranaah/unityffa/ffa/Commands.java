...
import dev.dammnranaah.ffa.arenas.coolarenas.CoolArenaManager;
import dev.dammnranaah.ffa.kits.Kits;
import dev.dammnranaah.ffa.spawnitems.Items;
import dev.dammnranaah.ffa.stats.edit.EditStats;
...
public class Commands implements CommandExecutor, TabCompleter {
        ...
        switch (subCommand) {
            ...
            case "kits":
                if (Bukkit.getServer().getPluginManager().isPluginEnabled("dammnranaahkits")) {
                    sender.sendMessage(formatColors(prefix + "&7Hooked into dammnranaahkits, Please use the command /kits to manage the kits."));
                    return true;
                }
                if (args.length < 2) {
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            ...
            List<String> completions = new ArrayList<>();
            if (Bukkit.getServer().getPluginManager().isPluginEnabled("dammnranaahkits")) {
                completions.addAll(dammnranaahkitsAPI.getKitsAPI().listKits());
            } else {
